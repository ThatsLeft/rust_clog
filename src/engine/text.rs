...
impl TextRenderer {
    pub fn new(texture_name: &str, glyph_w: f32, glyph_h: f32, atlas_cols: u32, atlas_rows: u32) -> Self {
        Self {
    ...
    pub fn set_scale(&mut self, scale: f32) { self.scale = scale.max(0.01); }
    pub fn set_spacing(&mut self, spacing: f32) { self.spacing = spacing; }
    ...
    pub fn draw_text_screen(&self, renderer: &mut Renderer, camera: &mut crate::engine::Camera2D, screen_pos: Vec2, text: &str) {
    ...
    pub fn draw_top_left(&self, renderer: &mut Renderer, camera: &mut crate::engine::Camera2D, offset_px: Vec2, text: &str) {
        self.draw_text_screen(renderer, camera, offset_px, text);
    }

    pub fn draw_top_right(&self, renderer: &mut Renderer, camera: &mut crate::engine::Camera2D, offset_px: Vec2, text: &str) {
