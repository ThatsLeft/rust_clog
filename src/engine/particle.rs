...
impl ParticleSystem {
    ...
    pub fn with_velocity_direction(
        mut self,
        dir: Vec2,
        speed_min: f32,
        speed_max: f32,
        ...
        self.velocity_spec = ParticleVelocitySpec::Direction {
            dir,
            speed_min,
            speed_max,
            spread_rad,
    ...
    pub fn with_velocity_radial(mut self, speed_min: f32, speed_max: f32) -> Self {
        self.velocity_spec = ParticleVelocitySpec::Radial {
            speed_min,
            speed_max,
        };
        self
    ...
    pub fn set_acceleration(&mut self, accel: Vec2) {
        self.global_accel = accel;
    }
    ...
    fn get_random_color(&self) -> Vec4 {
        ...
        match &self.color_spec {
            ...
            ParticleColorSpec::Range { min, max } => {
                let (rx0, rx1) = if min.x <= max.x {
                    (min.x, max.x)
                } else {
                    (max.x, min.x)
                };
                let (ry0, ry1) = if min.y <= max.y {
                    (min.y, max.y)
                } else {
                    (max.y, min.y)
                };
                let (rz0, rz1) = if min.z <= max.z {
                    (min.z, max.z)
                } else {
                    (max.z, min.z)
                };
                let (rw0, rw1) = if min.w <= max.w {
                    (min.w, max.w)
                } else {
                    (max.w, min.w)
                };
    ...
    fn next_velocity(&self) -> Vec2 {
        ...
        match &self.velocity_spec {
            ...
            ParticleVelocitySpec::Direction {
                ...
                let base = if dir.length_squared() > 0.0 {
                    dir.normalize()
                } else {
                    Vec2::new(1.0, 0.0)
                };
